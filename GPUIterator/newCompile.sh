...
nvcc --shared -o libs/libadd.so kernels/GPU_queens_kernels.cu  --compiler-options '-fPIC -O3' -I${CUDA_HOME}/include -L${CUDA_HOME}/lib -lcudart
...
chpl -Llibs -ladd -lutil -lGPUAPI -M modules -M ${GPUITE_HOME}/src --fast -s timeDistributedIters=true -s infoDistributedIters=true -s debugGPUIterator=true ${GPUITE_HOME}/src/GPUAPI.h newMain.chpl -o  newQueens.out
