module queens_aux{
	...
	proc queens_parser(const size: uint(16)){
		...
		writeln("\tNumber of solutions: ", metrics[1]);
		writeln("\tTree size: ", metrics[2]);
		writeln("Elapsed time: ", timer.elapsed()*1000, " ms"); // Print elapsed time
		writeln("Rate: ", metrics[2]/timer.elapsed(), "n/s\n\n"); // Print elapsed time
	...
	proc queens_print_serial_report(timer: Timer, size: uint(16), metrics: (uint(64),uint(64)),
        ...
        var number_of_solutions = metrics[1];
        var parallel_tree_size =  metrics[2];
	...
	proc queens_mlocale_print_metrics(size: uint(16), ref metrics: (uint(64),uint(64)), 
        ...
        writef("\n\tNumber of solutions found: %u", metrics[1]);
