...
queens_multilocale_cuda: cuda dir
	...
	chpl $(QUEENS_MLOCALE_GPU_FLAGS) $(QUEENS_DEBUG_FLAGS) $(QUEENS_GPU_DEBUB_FLAGS) -s GPUCUDA=true -s GPUAMD=false -L$(LIBRARY_DIR) -lqueens -lutil -M $(CHPL_MODULES_DIR) --fast $(QUEENS_DEBUG_FLAGS) queens_GPU_CPU_distributed.chpl -o  $(BUILD_DIR)/queens_CUDA_GPU_CPU_distributed.out
	@echo
	@echo " ### Compilation done ### "
	$(shell sh ./ncomp.sh)
