module queens_mlocale_parameters_parser{
	...
	proc queens_mlocale_parameters_parser(
		...
		const lchunk: int, const mlchunk: int, const slchunk: int, 
		const flag_coordinated: bool = false,
		ref distributed_active_set: [] queens_node, 
		const Space: domain(?), ref metrics,
		ref tree_each_locale: [] uint(64),
		const pgas: bool, 
		const num_gpus: c_int, const CPUP: real,
