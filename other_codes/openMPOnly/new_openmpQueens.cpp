...
void queens_subtree_enumeration(const unsigned idx, const int N, const unsigned nPrefixes, 
    ...
    //    board[i] = _EMPTY_;
    //}
    board = root_prefixes->board;

    flag = root_prefixes->control;
...
void call_queens(const int size, const int initialDepth){
    ...
    for(unsigned long long subproblem = 0; subproblem<n_subproblems; ++subproblem){
        ...
        queens_subtree_enumeration(subproblem, size, n_subproblems, initialDepth, subproblems_pool+subproblem,vector_of_tree_size_h, solutions_h);
        #ifdef REPORT
